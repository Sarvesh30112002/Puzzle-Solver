//MD -> COST
//PuzzleBoard aaray stores puzzle states.
//The code checks if the current puzzle state matches the goal state using the `twodArraySequentiallyEqual` function.
//The `ParentIndex` array is used to trace back the path from the goal state to the initial state.
//The code keeps track of the depth (the number of moves) from the initial state using the `DepthPuzzle` and `DepthPuzzleIndex` arrays.
